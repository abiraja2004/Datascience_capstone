...
OneT <- NGramTokenizer(corpus, Weka_control(min = 1, max = 1))
oneGM <- TermDocumentMatrix(corpus, control = list(tokenize = uniGramTokenizer))
twoGM <- TermDocumentMatrix(corpus, control = list(tokenize = biGramTokenizer))
...
save(fourgramlist,file="fourgram.Rda",envir = .GlobalEnv)
